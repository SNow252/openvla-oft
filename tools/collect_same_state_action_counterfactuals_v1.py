#!/usr/bin/env python3
"""
Collect same-state continuous action counterfactuals for LIBERO.

v1 goal:
  Replace the 6 fixed scripted candidates in v0/v0p1 with a continuous action
  distribution.

Core design:
  For the same initial state, generate target points by interpolating between
  default source bowl1 and language/counterfactual source bowl2:

      target_point = alpha * bowl2 + (1 - alpha) * bowl1 + noise

  Then execute a waypoint controller toward this target point for H steps.

This tests whether action-consequence prediction works beyond recognizing
a few fixed action templates.
...
def run_candidate(
    ...
    # For interpolation candidates, sample one fixed target point per candidate.
    target_point = None
...
def print_progress_preview(rows: List[Dict[str, Any]], max_seeds: int = 5) -> None:
    grouped: Dict[int, List[Dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault(int(row["init_state_idx"]), []).append(row)

    print("\n" + "=" * 100)
    print("[within-seed v1 progress preview]")
    for init_idx in sorted(grouped.keys())[:max_seeds]:
        print(f"\ninit_state_idx={init_idx}")
        rows_i = grouped[init_idx]

        # Sort: interpolation by alpha/noise first, then baselines.
        ...
        for row in sorted(rows_i, key=sort_key):
            ...
            noise = row.get("target_noise_std", float("nan"))
            print(
                f"  {row['candidate_name']:<32s} "
                f"fam={row['candidate_family']:<18s} "
                f"alpha={alpha!s:<6s} noise={noise!s:<6s} "
                f"p_bowl2={p2:+.4f} p_bowl1={p1:+.4f} adv={adv:+.4f}"
            )
    print("=" * 100)
...
def main() -> None:
    ...
    parser.add_argument("--alphas", type=str, default="-0.25,0,0.25,0.5,0.75,1.0,1.25")
    ...
    parser.add_argument("--include_plate", action="store_true", default=True)
    parser.add_argument("--include_stall", action="store_true", default=True)
    ...
    try:
        for local_i, init_state_idx in enumerate(range(args.start_init_state_idx, end_idx)):
            ...
            for cand_idx, spec in enumerate(specs):
                ...
                p2 = row.get("progress_to_bowl2", float("nan"))
                p1 = row.get("progress_to_bowl1", float("nan"))
                print(f"    progress_to_bowl2={p2:+.4f}, progress_to_bowl1={p1:+.4f}, source_adv={adv:+.4f}")
    ...
    write_summary_csv(all_rows, out_dir / "summary.csv")
    print_progress_preview(all_rows)
