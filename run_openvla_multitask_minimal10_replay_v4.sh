...
export OPENVLA_MT10_ROOT=$(ls -td /home/asd/projects/vla_lang_generalization/goal_controllability_verifier/data/fresh_policy_dumps_v4/openvla_multitask_minimal10_* | head -1)
export OPENVLA_MT10_MERGED="$OPENVLA_MT10_ROOT/policy_chunks_merged"

if [ ! -d "$OPENVLA_MT10_MERGED" ]; then
  echo "[ERROR] Missing merged dir: $OPENVLA_MT10_MERGED"
  exit 1
fi

echo "[INFO] OPENVLA_MT10_ROOT=$OPENVLA_MT10_ROOT"
echo "[INFO] OPENVLA_MT10_MERGED=$OPENVLA_MT10_MERGED"

run_replay_one_task() {
  local task_id="$1"
  local task_key="task${task_id}"

  export REPLAY_DIR=/home/asd/projects/vla_lang_generalization/goal_controllability_verifier/data/fresh_policy_replay_v4/openvla_${task_key}_minimal10_replay_140_$(date +%Y%m%d_%H%M%S)
  mkdir -p "$REPLAY_DIR"
...
PY
  ...
  echo
  echo "[INFO] dual_source tail:"
  tail -100 "$REPLAY_DIR/dual_source.log"
  echo
  echo "[INFO] table:"
...
echo
echo "[INFO] Done all OpenVLA multitask minimal replays."